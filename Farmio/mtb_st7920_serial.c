...
#define SCLK_PIN CYBSP_A0
#define CS_PIN CYBSP_A2
#define SID_PIN CYBSP_A1
#define RST_PIN CYBSP_A3
...
void SendByteSPI(uint8_t byte)
{
	for(int i=0;i<8;i++)
	{
		if((byte<<i)&0x80)
			{
				cyhal_gpio_write(SID_PIN, true);
			}

		else cyhal_gpio_write(SID_PIN, false);  // SID=0

		cyhal_gpio_write(SCLK_PIN, false);  // SCLK =0  OR SCK

		cyhal_gpio_write(SCLK_PIN, true);  // SCLK=1
...
{
    rslt = cyhal_gpio_init(SCLK_PIN, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, false);
    rslt = cyhal_gpio_init(CS_PIN, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, false);
    rslt = cyhal_gpio_init(SID_PIN, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, false);
